...
AESError AES::EncryptFile(std::string in_fname, std::string out_fname) {
	if( !initError.success ) {
		return initError;
	}

	FILE* fp_in, * fp_out;
	AESError err = FileOpen(&fp_in, in_fname, "rb");
	if( !err.success ) {
		return err;
	}
...
AESError AES::DecryptFile(std::string in_fname, std::string out_fname) {
	if( !initError.success ) {
		return initError;
	}
...
AESError AES::EncryptString(char* result, const char* data, unsigned int length) {
	if( !initError.success ) {
		return initError;
	}
...
AESError AES::DecryptString(char* result, const char* data, unsigned int length) {
	if( !initError.success ) {
		return initError;
	}
...
void AES::Init(Mode mode, const unsigned char* key, unsigned int keyBitLen, unsigned char* iv) {
	...
	unsigned int keyByteLen = keyBitLen / 8; // maybe 16, 32
	if( keyBitLen != 128 && keyBitLen != 256 ) {
		initError.success = false;
		std::stringstream ss;
		ss << "Unexpected key length. ";
		ss << "AES just only support 128, 256-bit, ";
	...
	for( unsigned int i = 0; i < keyByteLen; i++ ) {
		userKey[i] = key[i];
	}
...
AESError AES::FileOpen(FILE** fp, std::string fname, std::string mode) {
	AESError err;
	fopen_s(fp, fname.c_str(), mode.c_str());
	if( !( *fp ) ) {
		std::stringstream ss;
...
int AES::EncryptECB::Encrypt(char* res, const char* readBuf, unsigned int readSize) {
	int writeSize = 0;
	for( unsigned int pointer = 0; pointer < readSize; pointer += AES_BLOCK_SIZE ) {
