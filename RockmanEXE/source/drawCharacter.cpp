...
void DrawCharacter::LoadImages(){
	...
	if( isLoad ) {
		AppLogger::Error("draw character image is already loaded");
	}
	int tempNum[COL_MAX * 10];
	isLoad = true;

	// 数字関連
	std::string fname = def::IMAGE_FILE_PATH + "その他/number.png";
	LoadDivGraphWithErrorCheck(tempNum, fname, position, 10, 3, NUMBER_SIZE, 26);
	fname = def::IMAGE_FILE_PATH + "その他/number_2.png";
	LoadDivGraphWithErrorCheck(tempNum + 30, fname, position, 10, 1, NUMBER_SIZE, 20);
	...
	// チップコード
	fname = def::IMAGE_FILE_PATH + "その他/chip_code.png";
...
void DrawCharacter::DrawNumber(int x, int y, int data, DrawCharacter::Color color, int length){
	if( !isLoad ) {
		AppLogger::Error("draw character images are not loaded");
	}
...
void DrawCharacter::DrawChipCode(int x, int y, char code, int percent){
	...
	if( !( ( 'A' <= code && code <= 'Z' ) || code == '*' ) ) {
		AppLogger::Error("Invalied code %c will draw", code);
	}
